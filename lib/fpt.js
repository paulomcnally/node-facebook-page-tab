          ...
          if (!error && response.statusCode == 200) {
            ...
            async.parallel({
              ...
              application: function (callback) {
                request(util.format(graphUrl + 'fql?q=SELECT+app_id,app_name,namespace,display_name+FROM+application+WHERE+app_id=%d&access_token=%s', options.id, req.session.accessToken), function (error, response, body) {
              ...
              page: function (callback) {
                request(util.format(graphUrl + 'fql?q=SELECT+username,website+FROM+page+WHERE+page_id=%d&access_token=%s', req.session.pageId, req.session.accessToken), function (error, response, body) {
            ...
            function (err, results) {
              ...
              if (results.hasOwnProperty('application') && results.application.hasOwnProperty('data') && results.application.length > 0) {
                facebook.application = results.application.data[0];
              }

              if (results.hasOwnProperty('page') && results.page.hasOwnProperty('data') && results.page.length > 0) {
