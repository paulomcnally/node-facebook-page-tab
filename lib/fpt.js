...
module.exports.auth = function(app, options) {
  ...
  return function auth(req, res, next) {
    if (req.session.accessToken && req.session.facebook) {
      ...
      var urlAccessToken = util.format(graphUrl + 'oauth/access_token?code=%s&client_id=%d&redirect_uri=%s&client_secret=%s', req.query.code, options.id, encodeURIComponent(options.callback), options.secret);

      request(urlAccessToken, function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
          ...
          async.parallel({
            ...
            page: function (callback) {
              request(util.format(graphFql + 'fql?q=SELECT+username,website+FROM+page+WHERE+page_id=%d&access_token=%s', req.session.pageId, req.session.accessToken), function (error, response, body) {
          ...
          function (err, results) {
            ...
            res.redirect(util.format('https://www.facebook.com/%s/app_%d', options.pageId, options.id));
